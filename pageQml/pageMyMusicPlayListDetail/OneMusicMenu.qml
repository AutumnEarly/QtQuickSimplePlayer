...
QCMenu { // 右键菜单
    ...
    QCMenuItem { // 播放
        ...
        function playOneMusic() {
            let contentData = musicPlayListPage.contentData
    ...
    QCMenuItem { // 添加到指定歌单
        ...
        onClicked: {
            ...
            if(cmp.status === Component.Ready) {
                ...
                cmp.musicInfo = musicPlayListPage.contentData[rightMenu.playListCurrent]
                cmp.open()
            }
        }
    ...
    QCMenuItem { // 从此歌单移除
        ...
        function removePlayList() {
            ...
            let localCurrent = rightMenu.parent.localCurrent
            let index = rightMenu.playListCurrent
            let contentY = musicPlayListPageListView.contentY
            contentData.splice(index,1)
            musicPlayListPage.contentDataChanged()
            musicPlayListPageListView.contentY = contentY
            QCTool.writeFile(musicPlayListPage.playListInfo.dataPath, JSON.stringify(contentData))
        }
    }

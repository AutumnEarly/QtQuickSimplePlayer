...
Rectangle {
    ...
    width: historyRecordPage.width - 80
    height: 80
    radius: 12
    color: if(historyRecordPageListView.currentIndex === index) return  "#3F" + thisTheme.iconColor
    ...
    MouseArea {
        ...
        onDoubleClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                PageHistoryRecordJS.addPlayOneMusic(index,localCurrent)
            }
        }

        onClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                historyRecordPageListView.currentIndex = index
            }
            if( mouse.button === Qt.RightButton ) {
                PageHistoryRecordJS.openRightMenu(parent,mouseX,mouseY,index)
            }
        }
        onEntered: {

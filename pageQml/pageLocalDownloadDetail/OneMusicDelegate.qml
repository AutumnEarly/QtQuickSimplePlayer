...
Rectangle {
    ...
    width: musicLocalDownloadPage.width - 80
    height: 80
    radius: 12
    color: if(musicLocalDownloadPageListView.currentIndex === index) return  "#3F" + thisTheme.iconColor
    ...
    MouseArea {
        ...
        onDoubleClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                PageLocalDownloadJS.addPlayOneMusic(index,localCurrent)
            }
        }

        onClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                musicLocalDownloadPageListView.currentIndex = index
            }
            if( mouse.button === Qt.RightButton ) {
                PageLocalDownloadJS.openRightMenu(parent,mouseX,mouseY,index)
            }
        }
        onEntered: {
                ...
                QCToolTipButton {
                    ...
                    onClicked: {
                        if(isFavorite) {
                            ...
                            isFavorite = true
                            p_favoriteMusic.append(contentData[index])
                            iconSource = "qrc:/Images/myFavorite2.svg"
                        }
                    }
                    Component.onCompleted: {
                        isFavorite = p_favoriteMusic.favoriteIndexOf(contentData[index].id) < 0 ? false : true
                    }
                }

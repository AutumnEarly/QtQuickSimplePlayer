...
Rectangle {
    ...
    width: musicFavoritePage.width - 80
    height: 80
    radius: 12
    color: if(musicFavoritePageListView.currentIndex === index) return  "#3F" + thisTheme.iconColor
    ...
    MouseArea {
        ...
        onDoubleClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                ...
                PageMusicFavoriteJS.addPlayOneMusic(index,localCurrent)
            }
        }

        onClicked: function (mouse) {
            if( mouse.button === Qt.LeftButton ) {
                musicFavoritePageListView.currentIndex = index
            }
            if( mouse.button === Qt.RightButton ) {

                PageMusicFavoriteJS.openRightMenu(parent,mouseX,mouseY,index)
            }
        }
        onEntered: {
